...
export default class PopUpMenu extends React.Component {
    ...
    render() {
        return (
            <Menu renderer={Popover} rendererProps={{ placement: 'bottom', preferredPlacement: 'bottom', anchorStyle: { backgroundColor: 'white'} }}>
                <MenuTrigger style={{ padding: 15, }} customStyles={{ triggerTouchable: { activeOpacity: 1, underlayColor: 'transparent', } }}>
                    <Image style={{ width: 30, height: 4, resizeMode: 'contain' }}
                        source={require('../assets/images/3dot.png')} />
                </MenuTrigger>

                <MenuOptions
                    customStyles={{ optionWrapper: { justifyContent: 'space-between', }, optionText: { fontSize: 18 } }}
                    optionsContainerStyle={{ backgroundColor: 'transparent', }}
                    style={{ padding: 5, borderRadius: 10, backgroundColor: '#ffffffEE', }}>
                    <MenuOption onSelect={() => {this.props.navigation.navigate('EditFeed', { ImageUlr: this.props.img, content: this.props.text })}}>
                        <View style={{ flexDirection: 'row' }}>
                            <View style={{ width: 70 }}>
                                <Text style={{ fontSize: 16 }}>Edit</Text>
                            </View>
                            <Image style={{ width: 20, height: 15,resizeMode: 'contain', marginTop: 3 }}
                                source={require('../assets/images/pencil-3x.png')} />
                        </View>
                    </MenuOption>
                    <MenuOption onSelect={() => {this.createTwoButtonAlert(this.props.feed_id)}}>
                        <View style={{ flexDirection: 'row' }}>
                            <View style={{ width: 70 }}>
