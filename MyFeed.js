...
import { View, StyleSheet, Text, TouchableOpacity, Image, FlatList, RefreshControl, ActivityIndicator,Modal } from 'react-native';
...
import PopUpMenu from '../screens/MenuPopup';
...
export default class MyFeed extends React.Component {
  ...
  static navigationOptions = ({ navigation }) => ({
    ...
    headerTitle: <Text style={textHeader.header}>{`${Constants.my_feed}`.toUpperCase()}</Text>,
      ...
      <View style={{ padding: 10 }}>
        <View style={{ flexDirection: 'row', flex: 1 }}>
         {item.users.userProfilePicture==null ?
          ...
          <Image style={{ width: 42, height: 42, borderRadius: 42 / 2 }} source={require('../assets/images/admin.jpg')} resizeMode="contain" />
         : <Image style={{ width: 42, height: 42, borderRadius: 42 / 2 }}
            source={{ uri: item.users.userProfilePicture }} />
           
         }
          <View style={{ marginLeft: 10, flex: 1, }}>
            <Text style={{ color: '#293776', fontSize: 18 }}>{item.users.firstName}</Text>
            <Text style={{ color: '#ccc', fontSize: 16 }}>{item.created_at}</Text>
          
           
          </View>
          <PopUpMenu {...this.props} text={item.feed_content}
            img={item.feed_img_video} feed_id={item.id} />
        </View>
        <View style={{ marginTop: 5 }}>
          <Text style={{ color: '#444343', fontSize: 15, lineHeight: 22 }}>{item.feed_content}</Text>
        </View>
      </View>
      <TouchableOpacity
        onPress={() => { this.props.navigation.navigate('FeedDetails',{ ImageUlr: item.feed_img_video }) }}>
    ...
    <View style={{ flex: 1, backgroundColor: '#fff', marginVertical: 5 }}>
      <View style={{ flexDirection: 'row', }}>
        ...
        <View style={{ marginLeft: 18, justifyContent: 'center' }}>
          <Text style={{ color: '#293776', fontSize: 16, textAlign: 'center' }}>{item.name}</Text>
        </View>
      </View>
    </View>
  );
  ...
  ListEmpty = () => {
    return (
      <View style={{ justifyContent: 'center' }}>
        <Text style={{ textAlign: 'center', color: Colors.dark_gray }}>No Record Found</Text>
      </View>
    );
  };
